define({ 
  ...
  setDataToSeg:function(rowindex){
    try{
      var reverseData=[], orderedData=[];
      var reverseTmp={}, popedData={}, orderedTmp={};
      ...
      for(var i=this.routeId.length-1;i>=0;i--){
        ...
        if(Math.floor(this.hrs[i])<=0){
          ...
          reverseTmp={"key":this.routeId[i],"lblRoute":"Route"+(i+1),"lbldistnTime":this.kms[i]+" Miles, "+Math.round(this.min[i])+" min"};
        }else{
          this.view.lblTime.text=Math.round(this.hrs[i])+" hr";
          reverseTmp={"key":this.routeId[i],"lblRoute":"Route"+(i+1),"lbldistnTime":this.kms[i]+" Miles, "+Math.round(this.hrs[i])+" hr"};
        } 
        if(rowindex!==i){
      ...
      for(var j=0;j<=reverseData.length;j++){
        ...
        orderedTmp={"key":popedData.key,"lblRoute":"Route"+(j+2),"lbldistnTime":popedData.lbldistnTime};
